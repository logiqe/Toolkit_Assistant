...
BOARD_ID = binascii.hexlify(mac_address).decode('utf-8')
...
def save_wifi(ssid, password):
    ...
    microcontroller.nvm[0:128] = data + b'\x00' * (128 - len(data))

def get_saved_wifi():
    try:
        data = microcontroller.nvm[0:128].decode("utf-8").strip('\x00')
        if ":" in data:
            return data.split(":", 1)
    except:
        pass
...
# --- WIFI MANAGING AND PORTAL ---

class FauxDNS:
    def __init__(self, pool, ip_address):
        self.sock = pool.socket(pool.AF_INET, pool.SOCK_DGRAM)
        self.sock.settimeout(0)
        self.sock.bind(("", 53)) # Le port 53 est le port officiel du DNS
        self.ip_bytes = bytes(map(int, ip_address.split(".")))

    def poll(self):
        buf = bytearray(512)
        try:
            size, addr = self.sock.recvfrom_into(buf)
            if size > 0:
                self.repondre(buf[:size], addr)
        except OSError:
            pass # Rien à lire

    def repondre(self, requete, addr):
        # On fabrique une réponse qui dit "L'adresse que tu cherches est mon IP !"
        reponse = bytearray()
        reponse += requete[:2] # ID de la requête
        reponse += b"\x81\x80" # Flags: Réponse standard, pas d'erreur
        reponse += requete[4:6] # Nombre de questions
        reponse += b"\x00\x01\x00\x00\x00\x00" # 1 Réponse

        # On copie la question de la requête
        idx = 12
        while requete[idx] != 0:
            idx += requete[idx] + 1
        idx += 5
        reponse += requete[12:idx]

        # On ajoute notre réponse (Pointer, Type A, Class IN, TTL 60s, Longueur 4, Notre IP)
        reponse += b"\xc0\x0c\x00\x01\x00\x01\x00\x00\x00\x3c\x00\x04" + self.ip_bytes

        self.sock.sendto(reponse, addr)

ssid, password = get_saved_wifi()
connected = False
pool = None
...
if ssid and password:
    ...
    for attempt in range(3):
        try:
            ...
            t0 = time.monotonic()
            while not wifi.radio.ipv4_address and time.monotonic() - t0 < 15:
                time.sleep(0.5)
            if wifi.radio.ipv4_address:
                print("Connecté au Wi-Fi :", wifi.radio.ipv4_address)
                connected = True
    ...
    if not connected:
        print("WiFi inaccessible après 3 tentatives — lancement du portail")
        # On n'efface PAS le NVM ici

if not connected:
    print("Starting portal...")
    wifi.radio.start_ap("Toolkit_Setup", "")

    # IP fixe du point d'accès
    AP_IP = str(wifi.radio.ipv4_address_ap)
    print("AP IP:", AP_IP)

    # On met l'IP dans le nom du réseau pour que l'utilisateur sache où aller
    wifi.radio.stop_ap()
    wifi.radio.start_ap(f"Toolkit_Setup_{AP_IP}", "")

    pool = socketpool.SocketPool(wifi.radio)
    server = Server(pool, "/static", debug=True)

    CHAT_URL = f"https://toolkit-assistant.onrender.com/?board_id={BOARD_ID}"
    ...
    def base(request: Request):
        ...
        <head>
            ...
            <style>
                ...
                h1 {{ color: #FF8B33; }}
                input, button {{ margin: 10px; padding: 12px; width: 80%; max-width: 300px; font-size: 16px; border-radius: 8px; border: 1px solid #ccc; }}
                button {{ background-color: #FF8B33; color: white; border: none; cursor: pointer; }}
                .hint {{ color: #888; font-size: 13px; margin-top: 20px; }}
            </style>
        </head>
        <body>
            <h1>Toolkit Setup</h1>
            <p>Enter your WiFi credentials to connect the board.</p>
            <form action="/save" method="POST">
                <input type="text" name="ssid" placeholder="WiFi name" required><br>
                <input type="password" name="password" placeholder="WiFi password"><br>
                <button type="submit">Connect</button>
            </form>
            <p class="hint">Board ID: {BOARD_ID[:8]}</p>
        </body>
        </html>
        """
    ...
    def save(request: Request):
        ...
        # PAGE DE REDIRECTION MAGIQUE
        html_redirect = f"""
        <!DOCTYPE html>
        <html>
        <head>
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <!-- Redirection automatique vers ton FastAPI après 5 secondes -->
            <meta http-equiv="refresh" content="5;url={CHAT_URL}">
        </head>
        <body style="font-family: Arial; text-align: center; margin-top: 50px;">
            <h1>Configuration done!</h1>
            <p>The board is restarting and connecting to the network.</p>
            <p>You will be redirected to your Assistant in 5 secondes...</p>
            <a href="{CHAT_URL}">Click here if nothing is happening</a>
        </body>
        </html>
        """
        response = Response(request, body=html_redirect, content_type="text/html")
        return response

    ip_ap = str(wifi.radio.ipv4_address_ap)
    server.start(ip_ap, port=80)
    print(f"Serveur démarré sur http://{ip_ap}")

    dns_server = FauxDNS(pool, ip_ap) # <-- On allume notre faux DNS
    print(f"Serveur web et DNS lancés sur http://{ip_ap}")


    portal_start = time.monotonic()
    PORTAL_TIMEOUT = 120
    # Boucle infinie du portail captif : on attend que l'utilisateur valide le formulaire
    while True:
        server.poll()
        dns_server.poll()

        # Le redémarrage est géré proprement ici
        if need_reset:
            time.sleep(2) # Laisse le temps d'envoyer la page HTML au téléphone
            microcontroller.reset()

        # Si personne n'a soumis de credentials après 2 min → on retente le WiFi
        if time.monotonic() - portal_start > PORTAL_TIMEOUT:
            print("Timeout portail — nouvelle tentative WiFi...")
            microcontroller.reset()
...
def on_message(client, topic, message):
    ...
    try:
        ...
        if not validate_program(data):
            ...
            mqtt_client.publish(mqtt_publish_topic, json.dumps({
        ...
        if cmd.startswith("calibrate_"):
            ...
            if name in ANALOG_INPUTS:
                ...
                print("Calibrating:", name, "will start in 5s for", CALIBRATION_DURATION, "s")
            else:
        ...
        if "hardware_config" in data:
            init_hardware(data["hardware_config"])
        ...
        # We put everything back to zero and we receive a new message
        led1_index = 0
        led2_index = 0
        piezo_index = 0
...
client_id = f"pico_{BOARD_ID[:8]}"
mqtt_topic = f"toolkit/{BOARD_ID}/command"
mqtt_publish_topic = f"toolkit/{BOARD_ID}/sensors"

print("Attente réseau...")
time.sleep(8) # Laisse 3 secondes au Pico pour stabiliser son Wi-Fi

print("Vérification WiFi...")
if not wifi.radio.ipv4_address:
    print("WiFi perdu, reconnexion...")
    try:
        wifi.radio.connect(ssid, password)
        print("Reconnecté:", wifi.radio.ipv4_address)
    except Exception as e:
        print("Échec reconnexion WiFi:", e)
        microcontroller.reset()  # Repart du début proprement
...
mqtt_client, loop_timeout = Create_MQTT(client_id)
...
while True:
    try:
        if not mqtt_client.is_connected():
            print("Connection mqtt lost")
            # Vérifier d'abord le WiFi
            if not wifi.radio.ipv4_address:
        ...
        for name, comp in hw_inputs.items():
            ...
            if comp["type"] == "touch":
                ...
                raw_val = 1 if comp["obj"].value else 0  # Pull.UP : False = pressé
            ...
            if cal_input == name:
                ...
                if cal_start <= now <= cal_end:
                    # On enregistre le min et le max réels vus par le capteur
        ...
        if cal_input is not None and time.monotonic() > cal_end:
            ...
            # On envoie le résultat au serveur pour que l'IA le voie
            calibration_result = {
            ...
            # On réinitialise l'état de calibration
            cal_input = None
